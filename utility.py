...
for _ in range(NUM_PROPERTIES):
    ...
    cursor.execute(
        "INSERT INTO Property (amenities, propertyID, address, Name, communityAnnouncements) VALUES (%s, %s, %s, %s, %s)",
        (amenities, property_id, address, name, announcements)
    )
...
for _ in range(approved_count):
    ...
    property_unit_map[unit_id] = property_id
    rent_paid = fake.boolean()
    date_rand = fake.date_between(start_date='-1m', end_date='+1m')
    rent_due = datetime.combine(date_rand, time(23, 59, 59))
    ...
    rented_unit_ids.append(unit_id)
    cursor.execute(
...
for user in unit_users:
    ...
    property_id = property_unit_map[unit_id]
    cursor.execute(
        "UPDATE Customer SET unitid = %s WHERE UserID = %s",
        (unit_id, user)
    )
    cursor.execute(
    "UPDATE Customer SET propertyID = %s WHERE unitID = %s",
    (property_id, unit_id)
    )
...
cursor.execute("SELECT UnitId, UserID, PropertyID FROM Unit WHERE isRented=TRUE")
unit_info_mapping = {unit_id: (user_id, property_id) for unit_id, user_id, property_id in cursor.fetchall()}
...
for _ in range(NUM_MAINTENANCE_REQS):
    ...
    unit_id = random.choice(rented_unit_ids)
    user_id, property_id = unit_info_mapping[unit_id]


    cursor.execute(
        "INSERT INTO MaintenanceRequest VALUES (%s, %s, %s, %s, %s, %s)",
        (is_dealt_with, request_id, timestamp, property_id, unit_id, user_id)
    )
